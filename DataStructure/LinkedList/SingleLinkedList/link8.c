...
void insertAscending()
{
    ...
    // If the node that we are going to insert happen to be the first node
    if(prev == NULL)
    {
        n->next = curr;
        HEAD = n;
    }
    else if(curr == NULL) // If the node that we are going to insert happen to be the last node
    {
        prev->next = n;
    }
    else
...
void displayRecursive(struct node *n)
{
    if(n==NULL) return;
    printf("%d\n",n->data);
...
void insertcenter()
{
    ...
    struct node *p;
    int center=countnodes()/2;
    int count,data;
    prev = NULL;
    curr = HEAD;
   
    printf("Enter data to insert at center :");
    scanf("%d", &data); 
    ...
    for(count =1; count <= center; count++)
    {
        prev = curr;
        curr = curr->next;
    }
...
int main(void)
{
    ...
    do
    {
        ...
        printf("12.DISPLAY RECURSIVE\n");
        printf("13.No.of.Nodes\n");
        printf("14.InsertCenter\n");
        printf("0.EXIT\n");
        ...
        switch(op)
	{
            ...
            case 11: insertDescending();
                     break;
            case 12: displayRecursive(HEAD);
                     break;
            case 13: printf("No of Nodes = %d\n", countnodes());
                     break;
            case 14: insertcenter();
                     break;
            case 0: deleteAll();
                    break;
	}
