...
int main(int argc, char *argv[])
{
    ...
    unsigned char *buffer, *ptrRed, *ptrGreen, *ptrBlue;
    ...
    for(row = 0; row < height; row++)
    {
        for(col = 0; col < width; col++)
        {
            ...
            fwrite(ptrRed,   sizeof(unsigned char), 1, fw);
            fwrite(ptrGreen, sizeof(unsigned char), 1, fw);
            fwrite(ptrBlue,  sizeof(unsigned char), 1, fw);
            ptrRed   += 2 * 3;
            ptrGreen += 2 * 3;
            ptrBlue  += 2 * 3;
        }
