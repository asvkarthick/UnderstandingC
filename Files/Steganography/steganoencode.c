...
int main(int argc, char *argv[])
{
    ...
    unsigned char color;
    char msg[100];
    int msglength, hiddenbitscount = 0;
    ...
    for(row = 0; row < height; row++)
    {
        for(col = 0; col < width * 3; col++)
        {
            ...
            if(hiddenbitscount < msglength)
            {
                int byte = hiddenbitscount / 8;
                int bit  = hiddenbitscount % 8;
                int hiddenbit = (msg[byte] & (1 << bit)) >> bit;
                if(hiddenbit == 0)
                {
                    color = color & 0xFE; // color = color & ~0x1;
                }
                else // hiddenbit == 1
                {
                    color = color | 0x01;
            ...
            fwrite(&color, sizeof(unsigned char), 1, fw);
        }
    }
